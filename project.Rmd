...
ggplot(rita, aes(wday, count)) + 
  geom_boxplot()
ggplot(gian, aes(wday, count)) + 
  geom_boxplot()
```
```{r}
ggplot(filter(rita, ! (wday %in% c("sab", "dom",'mer'))), aes(start_game, score)) + 
  geom_line() +
...
full_table %>%
  arrange(start)%>%
  filter(game=="6267d060afe2b9dff26e8987") %>%
  mutate(time = as.numeric(end-start) ,id=1:25)%>%
  ggplot()+
