...
plot(hclust(dist(matriz_emojis,method = 'euclidean')))
...
distance<-dist(matriz_emojis,method = 'euclidean',diag = TRUE )
#dist(scale(emo1_emo2))
#print(distance)
#print(matriz_emojis)
...
```
```{r}
comp= as.data.frame(matriz_emojis)%>%filter(Freq!=0)
comp%>%graph_from_data_frame()%>%
  ggraph(layout = "fr") +
