...
```{r include=FALSE}
mytheme<-theme_void()+
        theme(panel.background = element_rect(fill = "#0c1329",colour = NA_character_),# necessary to avoid drawing panel outline
...
ggplot(data=user_summarise)+
  geom_bar(mapping = aes(x = user, fill = sex))+
  theme_void()+
...
full_table%>%
 ggplot(aes(emoji1,emoji2))+
  geom_bin_2d()+scale_fill_distiller(palette = "RdPu")+theme_void()+mytheme+themeBars
full_table%>%filter(!game%in% more_games$game)%>%select(emoji1,emoji2)%>%
 ggplot(aes(emoji1,emoji2))+
  geom_bin_2d()+scale_fill_distiller(palette = "RdPu")+theme_void()+mytheme+themeBars
emojis =sort(unique(full_table$emoji1))


full_table%>%group_by(emoji1,emoji2)%>%summarise(count=n())%>%filter(emoji1==emoji2)
...
n<-full_table%>%filter(!game%in% more_games$game)%>%select(emoji1,emoji2)
...
```{r}
more_games_2=more_games%>%mutate(duration= as.numeric(end_game-start_game))
more_games_2%>%
...
mod2 <- lm(score ~ exp(duration), data = more_games_2)

mod3 <- lm(duration~score, data = more_games_2)
...
# less games
less_games%>%
  mutate(duration= as.numeric(end_game-start_game),score=score)%>%
  ggplot(aes(duration,score))+
  geom_point()
...
```{r}
user_summarise%>%filter( last1Emoji == lastEmoji | last1Emoji == last2Emoji | last2Emoji == lastEmoji)%>%group_by(user)%>%summarise(n=n())
```
...
more_games%>%
  ...
  ggplot()+
  geom_boxplot(aes(id_game,score, group=id_game))
```
